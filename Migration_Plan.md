...
| 2026-03-16 | **Auto-breadcrumbs implemented:** Changed breadcrumbs from opt-in (`breadcrumbs: true` metadata required) to **opt-out** (auto-generated by default, suppress with `breadcrumbs: false`). Header now builds breadcrumbs automatically from the nav tree using pathname-based matching and `placeholders.json` for the home label ("AIG損保") and URL (`/sonpo`). Manual breadcrumb blocks in content still take priority — if a `breadcrumb` block exists in `<main>`, auto-generation is skipped. Removed manual breadcrumb blocks from all 8 T2 pages (16 content files + 1 .md) so they use auto-generated breadcrumbs. Fixed CSS spacing regression: `body[data-breadcrumbs]` (which adds 34px header height) is now only set when auto-breadcrumbs will render in the header, not when a manual breadcrumb block exists in `<main>`. |
...
| 2                              |                     | **Breadcrumb**                   |  —  |  ✓  |  ✓  |  ✓  |  ✓  |  ✓  |  ?  |  ?  |  ✗  | Auto-generated from nav tree by default (opt-out with `breadcrumbs: false`; manual block override available) |
...
**Default behavior: Auto-generated (no authoring required)**

Breadcrumbs are **generated automatically by default** on all pages. The header walks the nav tree to find the current page URL and traces back through parent `<li>` elements to build the breadcrumb trail. The home crumb label and URL are read from `/placeholders.json`:

```json
{ "Key": "breadcrumbs-home-label", "Text": "AIG損保" }
{ "Key": "breadcrumbs-home-url", "Text": "/sonpo" }
```

No metadata or block authoring is needed — breadcrumbs just appear. Authors only need to take action to **suppress** or **override** them.

**To suppress breadcrumbs on a page:**
...
**To override with custom breadcrumbs (for custom labels or pages not in the nav tree):**

Add a manual `breadcrumb` block in the page content:
```
| breadcrumb |                |
| AIG損保     |                |
| 個人向け保険 |                |
...
**Priority logic:** If a manual `breadcrumb` block exists on the page, the auto-generated breadcrumb is suppressed — the manual block always wins. Both produce identical visual output (same `>` separator, navy links, gray text, 12px font, hidden on mobile <=768px).
...
- Auto-generated: built in `header.js` (`buildBreadcrumbsFromNavTree` + `buildBreadcrumbs`), appended inside the header wrapper; uses pathname-based URL matching with `/content` prefix normalization for local dev compatibility; falls back to `og:title` if the current URL isn't found in the nav tree
- Manual block: built in `breadcrumb.js`, rendered in the page content area as the first section
- Homepage suppression: auto-breadcrumbs are automatically suppressed when there's only a single crumb (i.e., on the homepage itself)
- CSS spacing: `body[data-breadcrumbs]` attribute (adds 34px to header height) is only set when auto-breadcrumbs will render in the header, not when a manual breadcrumb block exists in `<main>`
- The `placeholders.json` file must be uploaded to da.live for the deployed site to read the home label correctly
- Some page types (landing pages, brand microsites) can intentionally omit breadcrumbs via `breadcrumbs: false` metadata
